 ...
 * @generated
 */
public class RespondedExistenceReorientCommand extends EditElementCommand {
	 ...
	 * @generated
	 */
	public RespondedExistenceReorientCommand(ReorientRelationshipRequest request) {
	...
	public boolean canExecute() {
		if (false == getElementToEdit() instanceof RespondedExistence) {
			return false;
		}
	...
	protected boolean canReorientSource() {
		...
		if (getLink().getToActivity().size() != 1) {
			return false;
		}
		Activity target = (Activity) getLink().getToActivity().get(0);
		...
		return DeclareBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistRespondedExistence_4003(container, getLink(),
						getNewSource(), target);
	}
	...
	protected boolean canReorientTarget() {
		...
		Declare container = (Declare) getLink().eContainer();
		return DeclareBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistRespondedExistence_4003(container, getLink(), source,
						getNewTarget());
	}
	...
	protected CommandResult reorientTarget() throws ExecutionException {
		getLink().getToActivity().remove(getOldTarget());
		getLink().getToActivity().add(getNewTarget());
		return CommandResult.newOKCommandResult(getLink());
	}
	 ...
	 * @generated
	 */
	protected RespondedExistence getLink() {
		return (RespondedExistence) getElementToEdit();
	}
