...
public class ChoiceItemSemanticEditPolicy extends
	...
	public ChoiceItemSemanticEditPolicy() {
		super(DeclareElementTypes.Choice_2002);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (DeclareVisualIDRegistry.getVisualID(incomingLink) == InputRelationEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (DeclareVisualIDRegistry.getVisualID(outgoingLink) == OutputRelationEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
